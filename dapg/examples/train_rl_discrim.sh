...
for i in ${!methods[@]}; do
    for j in ${!envs[@]}; do
        nohup python -W ignore job_script.py --discriminator_reward --warm_up 500 --output results/$DATE/${methods[$i]}_${envs[$j]}_discrim_exp --config cfg/${methods[$i]}_${envs[$j]}.txt  --record_video --save_id $DATE --wandb_activate --wandb_entity quantumiracle >> log/$DATE/${methods[$i]}_${envs[$j]}_discrim.log &
